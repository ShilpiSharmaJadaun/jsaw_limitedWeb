...
class ReceivedObservationPage extends StatefulWidget {
  const ReceivedObservationPage({super.key});

  @override
  State<ReceivedObservationPage> createState() => _ReceivedObservationPageState();
}

class _ReceivedObservationPageState extends State<ReceivedObservationPage>
    with AutomaticKeepAliveClientMixin {
  // Keep this tab's State (and its blocs) alive while it sits inside the
  // TabBarView, so dialogs opened from it don't end up referencing a disposed
  // State when the tab is briefly rebuilt.
  @override
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  ElevatedButton.icon(
                    onPressed: () {
                      uniqueIdBloc.initState(
                          responsibilityEnggCode: window.localStorage.getItem('kEmployeeCode') ?? "");
                      openFilterDialog();
                    },
                    style: ElevatedButton.styleFrom(
                      backgroundColor: kcvoilet,
                  ...
                  const SizedBox(width: 8),
                  ElevatedButton.icon(
                    onPressed: model.totalItems != 0 && currentPage < model.totalPages - 1
                        ? _nextPage
                        : null,
                    style: ElevatedButton.styleFrom(
                      backgroundColor: kcvoilet,
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Row(
                                ...
                                children: [
                                  _buildIconBadge(
                                      Icons.build_outlined, kcStatPurple),
                                  const SizedBox(width: 8),
                                  ...
                                  IconButton(
                                    tooltip: 'View / Edit',
                                    onPressed: () async {
                                      final shouldInit = await Navigator.of(context).push(MaterialPageRoute(
                                        builder: (context) =>
                                            EditReceivedObservationsPage(
                                          uniqueId: item.uniqueIdentificationNumber,
                                          raisedBy: item.observationRaisedByEmpUnqId,
                                          department: item.stationName,
                                          ...
                                          name: item.observationRaisedBy,
                                          raisedDate: item.raisedDate,
                                          wrkGrp: item.wrkGrp,
                                          deptCode: item.plantDeptCode,
                                          stateCode: item.stationCode,
                                          plant: item.plantDept,
                                          raisedByEmpID: item.observationRaisedByEmpUnqId,
                                          status: item.status,
                                          imageNumber: item.imageNumber,
                                          correctiveMeasures: item.correctiveMeasure,
                                          targetDate: item.observationCompletionTargetDate,
  ...
  // ─── Safety-remark-style card helpers ───
    ...
    return showDialog(
      ...
      builder: (ctx) => ObservationFilterDialog(
        ...
        onApply: () {
          ...
          // Reload with the selected filters.
